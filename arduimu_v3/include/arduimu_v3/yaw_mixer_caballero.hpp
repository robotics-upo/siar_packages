#ifndef __YAW_MIXER_CABALLERO_HPP__
#define __YAW_MIXER_CABALLERO_HPP__

#include <Eigen/Core>
#include <Eigen/Dense>
#include "yaw_mixer.hpp"

//! @class This This class estimates the yaw angle by adding a k 
//!                   as a function of the gyro and ticks rate
class YawMixerCaballero:public YawMixer
{
public:
  YawMixerCaballero(double hz) {
    init(hz);
  }
  
  //! @brief Default initializing function
  //! @param  hz Fitering frequency in Hz
  virtual void init(double hz);
  
  //! @brief EKF correction stage based on gyro rate and update rate
  //! @param gyroRate gyro rates in rad/s
  //! @return The yaw estimation
  virtual double update(double gyro_rate, double dt = -1.0);
  
  //! @brief EKF correction stage based on ticks rate 
  //! @param tickRate Yaw rate of tick estimation in rad/s
  virtual double correct(double ticks_rate);
  
protected:
  Eigen::MatrixXd P;        // Cov Matrix
  double min_tic_dev_, tic_prop_dev_; // The noise on the tick measure depends on the rate
  
};

void YawMixerCaballero::init(double hz)
{
  // Initial values
  rate_ = bias_ = yaw_ = 0.0;
  
  // Noise related to measures
  tic_prop_dev_ = 0.5; // Instead than a constant noise for tick, it will depend on the yaw rate n=a+b*rate
  min_tic_dev_ = 0.01;
  gyr_dev_ = 0.005;
  bia_dev_ = 0.005 / hz;
  T_ = 1 / hz;
  
  // Initialize covariance matrix
  P.setIdentity(3, 3);
  P(0,0) = 0.000001*0.000001;
  P(1,1) = 0.01*0.01;
  P(2,2) = 0.001*0.001;
}

double YawMixerCaballero::update(double gyro_rate, double dt)
{
  // Prediction stage
  if (dt > 0.0) {
    T_ = dt;
  }

  // Compute matrix F 
  Eigen::Matrix<double, 3, 3> F;
  F(0,0) = 1, F(0,1) = T_, F(0,2) = 0;
  F(1,0) = 0, F(1,1) = 0 , F(1,2) = -1;
  F(2,0) = 0, F(2,1) = 0 , F(2,2) = 1;
  
  // Update covariance matrix
  P = F*P*F.transpose();
  P(1,1) += gyr_dev_*gyr_dev_;
  P(2,2) += bia_dev_*bia_dev_;
  
  // Update state vector
  yaw_ = yaw_ + T_ * rate_;
  rate_ = gyro_rate - bias_;
  
  return yaw_;
}

// EKF correction stage based on ticks rate
double YawMixerCaballero::correct(double tick_rate) {
  // Create measurement jacobian H
  Eigen::Matrix<double, 1, 3> H;
  H(0,0) = 0.0;  H(0,1) = 1.0;  H(0,2) = 0.0;
  
  // Compute measurement noise jacoban R
  Eigen::Matrix<double, 1, 1> R;
  tic_dev_ = min_tic_dev_ + tic_prop_dev_ * fabs(tick_rate);
  R(0,0) = tic_dev_*tic_dev_;

  // Compute innovation matrix
  Eigen::Matrix<double, 1, 1> S;
  S = H*P*H.transpose()+R;
  
  // Compute kalman gain
  Eigen::Matrix<double, 3, 1> K;
  K = P*H.transpose()*S.inverse();

  // Compute mean error
  double y = tick_rate - rate_;
  
  // Compute new state vector
  yaw_ += K(0,0)*y;
  rate_  += K(1,0)*y;
  bias_  += K(2,0)*y;
	      
  // Compute new covariance matrix
  Eigen::Matrix<double, 3, 3> I;
  I.setIdentity(3, 3);
  P = (I-K*H)*P;
  
  return yaw_;
}

#endif
