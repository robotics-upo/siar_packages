...
def callback(data):
  ...
  try:
    ...
    pos, ori = tf_listener.lookupTransform("/odom", "/base_link", t)
...
def gas_callback(data):
  try:
    t = tf_listener.getLatestCommonTime("/odom", "/base_link")
    pos, ori = tf_listener.lookupTransform("/odom", "/base_link", t)
    gas_text_file.write("%f %f %f %f %f %f %f %f %f\n"%(pos[0], pos[1], data.O2_conc, data.H2S_conc, data.CO_conc, data.CH4_conc, data.temperature, data.pressure, data.RH))
    print("Gas measure: %f %f %f %f %f %f %f %f %f\n"%(pos[0], pos[1], data.O2_conc, data.H2S_conc, data.CO_conc, data.CH4_conc, data.temperature, data.pressure, data.RH))
  except TfError:
...
if __name__ == '__main__':
  ...
  gas_text_file.write("% x y o2 h2s co ch4 temp pressure RH\n");
  rospy.spin()
  text_file.close()
